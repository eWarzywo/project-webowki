...
export function Card({
    ...
    const last3DataRecords = () => {
        ...
        if (loading) {
            return (
                <div className="text-[#A1A1AA] text-sm flex justify-center items-center w-full py-4">Loading...</div>
            );
        }
        if (error) {
            return (
            ...
            return recordsToShow.map((record) => (
                <span key={`${DataType[dataType]}-${record.id}`}>
                    <Link
                        ...
                        className="flex justify-center items-center w-full hover:bg-zinc-800 rounded-md p-1"
                    >
    ...
    return (
        <div className="flex flex-col justify-center items-start w-full sm:w-1/2 lg:w-1/3 rounded-xl border border-[#27272A]">
            <div className="flex flex-col justify-center items-start p-4 sm:p-6 w-full">
                <div className="flex justify-between items-center w-full">
                    <h1 className="text-xl sm:text-2xl text-[#FAFAFA] font-semibold gap-[10px]">{title}</h1>
                </div>
                <h3 className="gap-[10px] mt-1.5 font-normal text-[#A1A1AA] text-xs sm:text-sm">{subtitle}</h3>
            </div>
            <div className="w-full flex flex-col px-4 sm:px-6 pb-4 sm:pb-6 space-y-4">{last3DataRecords()}</div>
        </div>
    );
}
